...
class CachedStore(object):
    ...
    def __init__(self, store_config, logging_config):
        self._cache = base.Client((store_config.get('cache_host'), store_config.get('cache_port')),
                                  timeout=store_config.get('cache_timeout') or 15,
                                  connect_timeout=store_config.get('cache_connection_timeout') or 45)
        self._store = redis.StrictRedis(host=store_config.get('store_host'), port=store_config.get('store_port'),
                                        ...
                                        socket_timeout=store_config.get('store_timeout') or 15,
                                        socket_connect_timeout=store_config.get('store_connection_timeout') or 45)
