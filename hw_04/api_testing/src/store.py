...
class CachedStore(object):
    ...
    def __init__(self, store_config, logging_config):
        self._cache = base.Client((store_config.get('cache_host'), store_config.get('cache_port')), timeout=15, connect_timeout=45)
        self._store = redis.StrictRedis(host=store_config.get('store_host'), port=store_config.get('store_port'), db=store_config.get('store_db'),
